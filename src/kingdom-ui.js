...
export function productionCounts(state,playerId) {
  ...
  return {counts,territories:cells.length,cities:cities.length,cityStrength:fiefs(state,playerId).reduce((sum,f)=>sum+f.strength,0),pending:cells.filter(c=>c.building?.farmType==='trading_post'&&!c.building.choice).length};
}
...
export function bindKingdomInspection(state,inspected) {
  ...
  const highlight=id=>{
    ...
    const overlay=board.querySelector('.connection-overlay');if(overlay)overlay.innerHTML='';
