...
import { copyOptions, copyChoiceValue, evaluateFinal, isCopy } from './scoring.js';
...
function recordedRulings(state) {
  ...
  const rows=Object.entries(d.rulings||{}).filter(([key])=>finished||key.startsWith('opportunist:')).map(([key,value])=>{
  ...
  for(const [id,target] of Object.entries(finished?d.copyResolutions||{}:{})) {
...
export function scoringPanel(state,{showPlayAgain=true}={}) {
  ...
  const copies=finished?'':state.players[0].parchments.filter(isCopy).map(c=>{
    ...
    const selected=ranked.find(o=>o.value===copyChoiceValue(d,c.instanceId));
    return `<label class="copy-choice">${esc(c.name)} · start with ${esc(state.players[options.playerId].name)}<select data-copy="${c.instanceId}" ${ranked.length?'':'disabled'}><option value="">${ranked.length?'Choose a parchment':'No valid parchment to copy'}</option>${ranked.map(o=>`<option value="${o.value}" ${selected===o?'selected':''}>${esc(o.path.map(c=>c.name).join(' → '))} — ${o.points===null?'points pending':o.points+' pts'} · ${o.complete?o.total+' total':'total pending'}</option>`).join('')}</select>${selected?`<small class="copy-card-detail">${esc(cardText(selected.card,state))}</small>`:''}</label>`;
  }).join('');
  const questions=finished?'':result.issues.filter(i=>i.kind!=='copy').map(issue=>`<div class="ruling"><p>${esc(issue.label)}</p><p class="muted">This case is still awaiting clarification. Your explicit ruling is recorded with this game.</p><select data-ruling="${issue.key}"><option value="">Choose a ruling</option>${issue.options.map(n=>`<option value="${n}">${n} points</option>`).join('')}</select></div>`).join('');
