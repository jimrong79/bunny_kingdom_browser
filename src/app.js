...
import * as hardBots from './bots-hard.js';
...
const botPolicy=()=>{
  const policy=state.botDifficulty==='hard'?hardBots:state.botDifficulty==='easy'?easyBots:normalBots;
  // Stamp the policy actually making the next decision, including resumed games.
  if(policy.strategyVersion)state.botStrategyVersion=policy.strategyVersion;
  return policy;
};
...
function setup() {
  ...
  document.querySelector('#setup').onsubmit = event => { event.preventDefault(); const f = new FormData(event.target); state = createGame(data, Number(f.get('bots')), f.get('seed') || Date.now(), f.get('playerName'),{expansion:f.get('expansion')}); state.botDifficulty=['easy','hard'].includes(f.get('difficulty'))?f.get('difficulty'):'normal'; botPolicy(); constructionUndo=[]; constructionReturn=null; selected = []; buildingId=null; targets=[]; inspected=null; error=''; render(); soundEffects.play('round'); };
...
function render() {
  ...
  app.innerHTML = `
    ...
    <p class="save-status muted">${saved?'Autosaved in this browser':'Browser storage is unavailable; keep this tab open to retain your game'} · ${state.botDifficulty==='hard'?'Hard (test)':state.botDifficulty==='easy'?'Easy':'Normal'} bots · Seed ${escape(state.seed)}</p>`;
