...
## Camp construction planning

Commit `61f730a` is the checkpoint before this change. Normal now evaluates each legal Camp location, and saving the Camp, together with its reserved buildings. It uses the same construction search as its building decisions, including placement restrictions, occupied slots, resource combinations, its own parchments, and District Coins. Easy keeps its existing Camp policy.

Consecutive Camp offers to the same player are considered in priority order. The search evaluates every legal location from each retained position and keeps four candidate plans for the next Camp. Buildings are only projected after the Camps; they do not occupy imaginary slots during subsequent Camp choices. An opponent's offer stops the joint search, and the bot recalculates when its next offer arrives. Earlier saved Camps stay in inventory but cannot be reused by the current search. This bounded search is a heuristic, not an exhaustive proof of optimal play.

The development case was Duke's round-2 Camp choices in the five-player expansion game `1788902885852`. The old bot chose C4-3 and D10 while holding a City 3 and Gold Farm. Normal now chooses C5 and C4-3, then builds City 3 at C6 and Gold Farm at F3: Harvest rises from 1 to 4, with the same two new Coins. Tests commit a reduced public position rather than the private game export. They also cover two-Camp bridges, intervening opponents, saved Camps, unavailable building slots, view immutability, and hidden-card independence.

Validation passed 107 Node tests, the exact saved-game decision through the browser's Continue button, and a complete Normal three-player expansion browser game through scoring and resume. A small comparison against the frozen checkpoint used two deals per player count with all challenger seats rotated: 18 candidate plus 6 control base games, and 10 candidate plus 2 control five-player expansion games. All 36 games passed legal-move and card-conservation checks. Mean margin changes were +8.75, -5.83, +3.50, and +2.10 for 2, 3, 4, and 5 players respectively; this sample does not establish an overall strength gain. The largest setback was inspected: a stronger first Harvest led to different later drafting, a 5-point lower final score, and the same winner. The five-player sample's 10 candidate Camp decisions averaged 198 ms with a 750 ms maximum on the development machine while other checks were running.
