...
The journal contains only changes to public construction data and lives in `ui.constructionUndo`, outside the game object passed to bots. It survives autosave/resume. Failed actions roll back atomically. Undo validates the saved action's round, player, game seed, phase, and resulting position before restoring anything.

**Done building** moves the journal into `ui.constructionReturn`, alongside a checkpoint of public construction data. **Back to building** is available until the human confirms the harvest. It restores the pre-confirmation phase, board, readiness, log, and resource choices, then restores the placement journal. Bot placements completed before confirmation remain in place; their resource assignments are recalculated when construction is confirmed again. Scores, District history, cards, and older buildings stay consistent through repeated returns. No decks or private hands enter this checkpoint or bot views. **Confirm & harvest** discards it; other rounds and changed positions reject stale checkpoints. Older resource-stage saves can reopen construction at the logged phase boundary, retaining their resource choices, but their previously discarded undo history cannot be recovered.

Run `python3 tests/browser_construction_undo.py --url http://localhost:8000/` with Playwright installed to check placements, repeat undo, save/resume, returning from resource selection, human/bot resource resets, harvest locks, older saves, mobile controls, Rainbow Coins and links, and Camp priority with bot responses. The synthetic fixtures retain the full physical card inventory. `tests/construction-undo.test.js` also checks all building categories, both Sky Tower endpoints, prior District history, previous-round protection, stale actions, and failed transactions.
